...
describe('Cursor adapter helpers', () => {
  ...
  test('resolves a cursor command with base args', () => {
    ...
    const expectedBaseArgs = resolved.command.endsWith('cursor')
      ? ['agent']
      : [];
    expect(resolved.baseArgs).toEqual(expectedBaseArgs);
  });
...
describe('Cursor session mapping', () => {
  ...
  test('ignores invalid session ids when updating the mapping', async () => {
    await withTempDataDir(async (dataDir) => {
      ...
      expect(existsSync(join(dataDir, CURSOR_SESSION_MAPPING_STORE_ID))).toBe(
        false,
      );
    });
  });
